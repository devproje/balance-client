	...
	return (
		<>
			...
			<div className="row">
				...
				<div className="obj">
					<h3>Price</h3>
					{repl(edit, <p className={`${data.buy ? "buy_spend" : "buy_earned"}`}>₩{data.price.toLocaleString()}</p>, <input type="number" defaultValue={data.price} required />)}
				</div>
			...
			<div className="title obj memo">
				<h3>Memo</h3>
				{repl(edit, <pre>{data.memo}</pre>, <div id="b_input_memo" contentEditable>{data.memo}</div>)}
			</div>
